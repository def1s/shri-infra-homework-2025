...
Полный адрес до деплоя - http://89.169.188.229/hw/store  
Если у тебя возникнут какие-то проблемы с запуском actions или созданием ПР, то напиши, пожалуйста, мне в телеграмм -> @def1s0  
Скорее всего actions запустить не выйдет не из collaborations :)  
